...
updateRss() {
	...
	output=$(sqlite3 "$DATABASE" "SELECT title, name, description, author, date, file FROM articles ORDER BY date DESC")
 
	echo "$output" | while IFS='|' read -r title name description author date file; do
		...
		printf "\t\t\t<content:encoded><![CDATA[$(sed 's/]]>/]]]]><![CDATA[>/' "$file")]]></content:encoded>\n"
		printf "\t\t</item>\n"
	done
